...
public final class StableIdentities {
    ...
    private static boolean provisionalIdentity(Job<?, ?> job) {
        NotificationJobIdentity property = job.getProperty(NotificationJobIdentity.class);
        return property != null
                && property.isProvisionalXmlLoad()
                && !property.isQuarantined()
                && !quarantineFile(job).exists()
                && !identityFile(job).exists();
    }
    ...
    public static String jobId(Job<?, ?> job) throws IOException {
        synchronized (job) {
            ...
            synchronized (IDENTITY_ALLOCATION) {
                ...
                if (provisionalIdentity(job)) throw new IOException("Notification identity allocation is pending");
                try (var ignored = hudson.security.ACL.as2(hudson.security.ACL.SYSTEM2)) {
                    for (Job<?, ?> other : Jenkins.get().getAllItems(Job.class)) {
                        if (other != job && id.equals(existingId(other))) {
                            if (provisionalIdentity(other))
                                throw new IOException("Notification identity allocation is pending");
                            quarantine(job);
                            quarantine(other);
