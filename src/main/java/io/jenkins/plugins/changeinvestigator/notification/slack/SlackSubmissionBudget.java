...
public final class SlackSubmissionBudget {
    private final NotificationStore store;

    public SlackSubmissionBudget(Path controllerRoot, UUID controllerId) throws IOException {
        store = new NotificationStore(controllerRoot.resolve("bci-slack-budget"), controllerId);
    }
    ...
    public synchronized Decision reserve(UUID destination, UUID delivery, long now) throws IOException {
        ...
        if (data.size() != 2
                ...
                || now < 0) throw new IOException("Invalid Slack submission budget");
        ...
        while (fields.hasNext()) {
            ...
            try {
                ...
                throw new IOException("Invalid Slack reservation");
            }
            if (!entry.getValue().isIntegralNumber() || entry.getValue().asLong() < 0)
                throw new IOException("Invalid Slack reservation clock");
