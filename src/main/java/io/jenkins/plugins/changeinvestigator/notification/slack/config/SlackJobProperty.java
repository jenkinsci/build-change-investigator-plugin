...
public final class SlackJobProperty extends JobProperty<Job<?, ?>> {
    ...
    public SlackJobProperty(String mode, String destinationIds, boolean recovery, boolean aiOnly, boolean mentions) {
    ...
    public SlackJobProperty(
            String mode, List<String> destinationIds, boolean recovery, boolean aiOnly, boolean mentions) {
        this(mode, destinationIds == null ? "" : String.join(",", destinationIds), recovery, aiOnly, mentions);
    }
    ...
    public static final class DescriptorImpl extends JobPropertyDescriptor {
        ...
        public String getDisplayName() {
            return "Build Change Investigator Slack policy";
        }
        ...
        public JobProperty<?> newInstance(StaplerRequest2 request, JSONObject form) throws FormException {
            ...
            try {
                SlackJobProperty property = request.bindJSON(SlackJobProperty.class, form);
                if (property.mode == Mode.CUSTOM) {
                    for (UUID id : property.selectedIds()) {
                        if (SlackConfiguration.get().approved(job, id).isEmpty())
        ...
        public List<SlackDestination> approvedFor(Job<?, ?> job) {
            ...
            return SlackConfiguration.get().approvedFor(job);
        }
    }
}
