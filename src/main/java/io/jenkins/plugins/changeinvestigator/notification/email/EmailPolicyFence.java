...
public final class EmailPolicyFence {
    private EmailPolicyFence() {}

    public record Fence(long at, int afterBuild) {}

    public static Fence read(Job<?, ?> job) throws IOException {
        var path = job.getRootDir().toPath().resolve("bci-email-disabled-after");
        ...
        if (!Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS) || Files.size(path) > 64)
            throw new IOException("Invalid Email policy watermark");
    ...
    public static void disabled(Job<?, ?> job) throws IOException {
        synchronized (job) {
            if (!Files.isRegularFile(job.getRootDir().toPath().resolve("bci-email-active"), LinkOption.NOFOLLOW_LINKS))
                return;
            Fence prior = read(job);
            int after = job.getLastBuild() == null ? 0 : job.getLastBuild().getNumber();
            new TextFile(job.getRootDir()
                            .toPath()
                            .resolve("bci-email-disabled-after")
                            .toFile())
